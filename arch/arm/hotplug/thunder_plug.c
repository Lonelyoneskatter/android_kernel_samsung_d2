 ...
 * Tuned to Dual Core CPUs.
 * This is being ported for dualcore devices such specifically the D2 
 *
 */
...
static int suspend_cpu_num = 1, resume_cpu_num = 2;
static int endurance_level = 0;
static int device_cpus = 2;
...
int tune_endurance(int endurance) {
	if(endurance == 2) {
           endurance = 0;
	   pr_info("%s: Endurance Level was dual-core, set it to default\n", THUNDERPLUG);
	}
        return endurance;
} 
...
{
	int cpu;
	for(cpu = 1; cpu < 2; cpu++) {    //Lets use all cores to wake-up device and get things going in Yureka?
		if(cpu_is_offline(cpu))
			cpu_up(cpu);
	}
}
...
MODULE_DESCRIPTION("Hotplug driver for OctaCore CPU, tuned to Dual-Core CPU");
